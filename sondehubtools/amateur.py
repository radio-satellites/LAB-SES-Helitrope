...
class Uploader(object):
    ...
    def __init__(
        ...
        if software_name:
            ...
            self.software_name = "dashboard"
            self.software_version = "0.3"
    ...
    #CUSTOM FUNCTIONS FOR DASHBOARD. MODIFIED FILE!

        def poll_telemetry(self,period,spacecraft):
            #period - 0, 15s, 1m, 30m, 1h, datatype string
            url = "https://api.v2.sondehub.org/amateur/telemetry?duration="+str(period)+"&payload_callsign="+str(spacecraft)
            x = requests.get(url) #poll
            return x.text
